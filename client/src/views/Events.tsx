...
const FALLBACK_EVENTS: Event[] = [
  { id: 1, icon: "📖", title: "Book Club: The Midnight Library", description: "Join us for a cozy discussion of Matt Haig's heartwarming novel.", date: "2026-04-15", time: "6:00 PM", location: "Reading Room B", tag: "Book Club", tagColor: "#C8B8E8", seats: 20, seatsLeft: 7 },
  { id: 2, icon: "✍️", title: "Creative Writing Workshop", description: "A hands-on workshop for aspiring writers.", date: "2026-04-20", time: "3:00 PM", location: "Workshop Hall", tag: "Workshop", tagColor: "#B8D8B8", seats: 15, seatsLeft: 3 },
  { id: 3, icon: "🎤", title: "Author Talk: Vaishnavi Patel", description: "Meet the author of Kaikeyi.", date: "2026-04-28", time: "5:30 PM", location: "Main Hall", tag: "Author Talk", tagColor: "#F0D8A8", seats: 50, seatsLeft: 22 },
  { id: 4, icon: "🧒", title: "Children's Story Hour", description: "A magical reading session for children aged 5–10.", date: "2026-05-03", time: "11:00 AM", location: "Children's Corner", tag: "Kids", tagColor: "#FAC8C8", seats: 30, seatsLeft: 18 },
  { id: 5, icon: "🔍", title: "Mystery Night: Guess the Ending", description: "Read the first half of a mystery novel.", date: "2026-05-10", time: "7:00 PM", location: "Reading Room A", tag: "Book Club", tagColor: "#C8B8E8", seats: 25, seatsLeft: 14 },
  { id: 6, icon: "🌍", title: "Travel Writing Seminar", description: "Learn to capture your adventures in words.", date: "2026-05-17", time: "2:00 PM", location: "Workshop Hall", tag: "Workshop", tagColor: "#B8D8B8", seats: 20, seatsLeft: 11 },
];
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.8, background: event.tagColor + "55", color: "#5A3A2A", padding: "3px 10px", borderRadius: 999, fontFamily: "'Lato',sans-serif", textTransform: "uppercase" }}>
            {event.tag}
          </span>
          <h3 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: 17, fontWeight: 600, color: "#3D1F15", margin: "6px 0 0", lineHeight: 1.3 }}>
            {event.title}
          </h3>
        </div>
        <span style={{ fontSize: 10, fontWeight: 700, background: "#FFF0E8", color: "#C4836A", border: "1px solid #F0DDD4", padding: "4px 10px", borderRadius: 999, whiteSpace: "nowrap", fontFamily: "'Lato',sans-serif" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
        {[{ icon: "📅", text: fmtDate(event.date) }, { icon: "🕐", text: event.time }, { icon: "📍", text: event.location }].map(({ icon, text }) => (
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4 }}>
        <div>
          <div style={{ width: 120, height: 5, background: "#F0DDD4", borderRadius: 999, overflow: "hidden", marginBottom: 4 }}>
            <div style={{ height: "100%", width: `${(event.seatsLeft / event.seats) * 100}%`, background: full ? "#D9BFB5" : urgent ? "#E8A080" : "#C4836A", borderRadius: 999, transition: "width .4s ease" }} />
          </div>
        ...
        <button
          ...
          style={{
            ...
            transition: "opacity .2s", opacity: hovered && !full ? 0.9 : 1,
          }}
        >
          {full ? "Full" : "Register →"}
        </button>
...
export default function Events() {
  ...
  const [registered] = useState<number[]>([]);
    ...
    <>
      ...
      <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", background: "linear-gradient(160deg,#FDF5EE 0%,#F9EEE4 40%,#F4E6D8 100%)", fontFamily: "'Lato',sans-serif", padding: "52px 24px 100px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <p style={{ fontSize: 11, letterSpacing: 4, color: "#C4836A", textTransform: "uppercase", marginBottom: 10, opacity: 0.8 }}>Community</p>
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: 44, fontWeight: 600, color: "#3D1F15", margin: 0 }}>Upcoming Events</h1>
          <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", color: "#A07060", fontSize: 16, marginTop: 10 }}>Gather, read, and grow together.</p>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginBottom: 40 }}>
          {tags.map((tag) => (
            <button key={tag} className="tag-pill" onClick={() => setFilter(tag)}
              style={{
                ...
                transition: "all .2s", boxShadow: filter === tag ? "0 4px 14px rgba(165,98,76,0.25)" : "none",
              }}
            >{tag}</button>
          ))}
        </div>
        ...
        {loading && (
          <p style={{ textAlign: "center", color: "#C4836A", fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: 18, marginTop: 40 }}>
            Loading events…
          </p>
        )}

        {!loading && (
          <div style={{ maxWidth: 980, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(420px, 1fr))", gap: 24 }}>
